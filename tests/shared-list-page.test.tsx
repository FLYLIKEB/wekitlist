...
describe('SharedListPage', () => {
  ...
  it('shows a home button in the shared list header that links to the fresh home page with a back path', async () => {
    render(<SharedListPage listId="list-1" />);

    const homeLink = await screen.findByRole('link', { name: '홈으로' });

    expect(homeLink).toHaveAttribute('href', '/?fresh=1&from=list');
